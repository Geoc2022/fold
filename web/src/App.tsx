...
export default function App() {
  ...
  // Derive identity from the sync payload; fall back to a local optimistic
  // person set at onboarding time until the first sync lands.
  const [optimisticMe, setOptimisticMe] = useState<Person | null>(null)
  const me = data?.me ?? optimisticMe

  // If we have an id but the server says the person doesn't exist, reset.
  useEffect(() => {
  ...
  if (!hasId || !me) {
    ...
    // Have an id, waiting for first sync.
    return (
